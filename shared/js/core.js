...
window.Settings = {
    ...
    openModal() {
        ...
        try {
            ...
            try {
                const wsLevelEl = document.getElementById('cfg-ws-level');
                if (wsLevelEl) wsLevelEl.value = String(this.data.wsLevel || 6);
            } catch (e) { }
            try {
                const wsShowDefsEl = document.getElementById('cfg-show-defs');
                if (wsShowDefsEl) wsShowDefsEl.checked = !!this.data.wsShowDefs;
            } catch (e) { }
            try {
                const memLevelEl = document.getElementById('cfg-memory-level');
                if (memLevelEl) memLevelEl.value = this.data.memoryLevel || '8';
            } catch (e) { }
            try {
                const memThemeEl = document.getElementById('cfg-memory-theme');
                if (memThemeEl) memThemeEl.value = this.data.memoryTheme || 'animals';
            } catch (e) { }
            try {
                const g2048SizeEl = document.getElementById('cfg-2048-size');
                if (g2048SizeEl) g2048SizeEl.value = String(this.data.g2048Size || 4);
            } catch (e) { }
            try {
                const hangmanMaxEl = document.getElementById('cfg-hangman-max');
                if (hangmanMaxEl) hangmanMaxEl.value = String(this.data.hangmanMax || 6);
            } catch (e) { }
    ...
    saveFromUI() {
        try {
            ...
            try {
                const wsLevelEl = document.getElementById('cfg-ws-level');
                if (wsLevelEl) this.data.wsLevel = parseInt(wsLevelEl.value, 10) || 6;
            } catch (e) { }

            try {
                const wsShowDefsEl = document.getElementById('cfg-show-defs');
                if (wsShowDefsEl) this.data.wsShowDefs = !!wsShowDefsEl.checked;
            } catch (e) { }

            try {
                const memLevelEl = document.getElementById('cfg-memory-level');
                if (memLevelEl) this.data.memoryLevel = memLevelEl.value || '8';
            } catch (e) { }

            try {
                const memCardSizeEl = document.getElementById('cfg-memory-card-size');
                if (memCardSizeEl) this.data.memoryCardSize = memCardSizeEl.value || (this.data.memoryCardSize || 'standard');
            } catch (e) { }

            try {
                const memThemeEl = document.getElementById('cfg-memory-theme');
                if (memThemeEl) this.data.memoryTheme = memThemeEl.value || 'animals';
            } catch (e) { }

            try {
                const g2048SizeEl = document.getElementById('cfg-2048-size');
                if (g2048SizeEl) this.data.g2048Size = parseInt(g2048SizeEl.value, 10) || 4;
            } catch (e) { }

            try {
                const hangmanMaxEl = document.getElementById('cfg-hangman-max');
                if (hangmanMaxEl) this.data.hangmanMax = parseInt(hangmanMaxEl.value, 10) || 6;
            } catch (e) { }
